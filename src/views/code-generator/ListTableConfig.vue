  ...
  >
    ...
    <el-table-column prop="width" align="center" label="operate" width="50">
      <template #default="{ row, $index }">
        <el-button text @click="deleteListItem(row, $index)">delete</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
...
const setListTableData = (checkColumnArr) => {
  checkColumnArr.forEach((fItem) => {
    ...
    if (!hasKey) {
      ...
      fItem.componentType = listTableComponentTypeMapping(fItem.dataType, fItem.columnComment, fItem.columnName) //data库和前端控件中的type做映射
